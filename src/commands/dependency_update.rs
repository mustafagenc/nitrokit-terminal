use colored::*;
use crate::utils::{read_file_to_string, log_info, log_warning, log_error, log_success};
...
pub fn update_dependencies() {
    ...
    println!();
    println!("{}", "🎉 All dependency operations completed successfully!".green().bold());
                ...
                Ok(package_json) => {
                    ...
                    if let Some(deps) = package_json["dependencies"].as_object() {
                        for (name, version) in deps {
                            println!("  {} -> {}", name.green(), version.as_str().unwrap_or("unknown"));
                        }
                    }
...
fn update_yarn_dependencies() {
    ...
    if !yarn_available {
        ...
        if Command::new("npx").arg("yarn").arg("--version").output().is_ok() {
    ...
    let yarn_cmd = if cfg!(target_os = "windows") && Command::new("yarn.cmd").arg("--version").output().is_ok() {
        "yarn.cmd"
    } else {
        "yarn"
    };
    ...
    match Command::new(yarn_cmd).arg("upgrade").output() {
        Ok(output) => {
            ...
            println!(" {}", format!("✅ Completed in {:.2}s", duration.as_secs_f64()).green());
...
fn update_npm_dependencies() {
    ...
    let npm_cmd = if cfg!(target_os = "windows") && Command::new("npm.cmd").arg("--version").output().is_ok() {
        "npm.cmd"
    } else {
        "npm"
    };
...
fn update_pnpm_dependencies() {
    ...
    let pnpm_cmd = if cfg!(target_os = "windows") && Command::new("pnpm.cmd").arg("--version").output().is_ok() {
        "pnpm.cmd"
    } else {
        "pnpm"
    };
